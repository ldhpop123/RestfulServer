...
http.createServer(async (req, res) => {
    try {
        ...
        if(req.method === 'GET') { // req.method가 GET이면 아래의 블럭 실행
            if (req.url === '/') { // 요청 UR이 '/'(루트)라면 다음 블록 실행
                ...
                // fs.readFile() -> about.html 제공
                // __dirname -> 현재 실행 중인 스크립트 파일의 디렉토리 이름을 나타내는 Node.js의 특별 변수
                // path.join(__dirname, 'about.html' -> 현재 스크립트 파일이 위치한 디렉토리와 요청된 URL을 결합하여 파일의 전체 경로를 생성
                // 요청된 URL에 해당되는 파일의 내용이 data 변수에 저장함
                res.writeHead(200, {'Content-Type':'text/html; charset=utf-8'});
                return res.end(data);
            ...
            if (req.url.startsWith('/user/')) { // 만약 요청 URL이 '/user/'로 시작한다면 아래의 불록을 실행
                ...
                return req.on('end', () => {
                    ...
                    // JSON.parse(body) -> 받은 데이터를 JSON파일로 파싱하여 JavaScript 객체로 변환
                    // name 속성을 추출하여 해당 속성 값을 users 객체로 변환
            ...
            if (req.url.startsWith('/user/')) { // 만약 요청이 ''/user/'로 시작한다면 아래의 불록을 실행
                const key = req.url.split('/')[2]; // 요청 URL에서 사용자의 고유 키를 추출
                // split('/') 함수는 URL을 '/' 기준으로 분할하고, 그 중에서 세 번째 요소를 key로 지정
                delete users[key]; // users 객체에서 key에 해당하는 속성을 삭제 -> 특정 사용자를 삭제하는 기능, key -> 해당 사용자의 고유한 식별자
                res.writeHead(200,{'Content-Type' : 'application/json; charset=utf-8'});
                return res.end(JSON.stringify(users));
            }
        }
        ...
        console.error(err);
        res.writeHead(500);
        res.writeHead(500, {'Cotent-Type':'text/plain; charset=utf-8'}); // 500 에러가 응답으로 전송됨.
        res.end(err.message);
    }
})
    .listen(8082, () => {
